...
public class SelectUserFrame {
	...
	private String columnNames[] = new String[] { "账号", "密码", "mac",
			"terminalid", "ter_loginno", "usercode", "username", "c_platlevel",
			"t_platlevel" };
	...
	public SelectUserFrame() {
		...
		jFrame.add(createMiddleJPanel(), BorderLayout.CENTER);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (str2.equals("user")) {
					...
					if (lists.size() > 0) {
						...
						JOptionPane.showConfirmDialog(jFrame, "没有这个用户!", "查询结果", JOptionPane.CLOSED_OPTION);
					}
					...
					if (lists.size() > 0) {
						...
						JOptionPane.showConfirmDialog(jFrame, "没有这个mac!", "查询结果", JOptionPane.CLOSED_OPTION);
					}
				}
				table.updateUI();
